...
class TestDatabaseExportImport:
    ...
    def test_single_transcription_export_import_json(
        self, temp_db, sample_transcription
    ):
        ...
        try:
            ...
            assert (
                original_data["metadata"]["filename"]
                == imported_data["metadata"]["filename"]
            )
            assert (
                original_data["metadata"]["file_hash"]
                == imported_data["metadata"]["file_hash"]
            )
            assert (
                original_data["metadata"]["language"]
                == imported_data["metadata"]["language"]
            )
            assert (
                original_data["metadata"]["full_text"]
                == imported_data["metadata"]["full_text"]
            )
            assert len(original_data["segments"]) == len(imported_data["segments"])

            for orig_seg, imp_seg in zip(
                original_data["segments"], imported_data["segments"]
            ):
    ...
    def test_beige_transcription_json_round_trip(
    ...
    def test_export_with_feed_metadata(self, temp_db, sample_transcription):
        ...
        transcription_id = temp_db.save_transcription(
            sample_transcription, model_name="medium", **feed_metadata
        )
        ...
        assert (
            loaded["metadata"]["feed_item_published"]
            == feed_metadata["feed_item_published"]
        )
