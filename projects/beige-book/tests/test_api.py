...
class MockSummary:
    def __init__(
        self, total_items=0, processed=0, skipped=0, failed=0, elapsed_time=0.0
    ):
...
def mock_transcription_result():
    ...
    segments = [
        Segment(start_ms=0, end_ms=2000, text="The stale smell of old beer lingers."),
        Segment(
            start_ms=2000, end_ms=4000, text="It takes heat to bring out the odor."
        ),
        Segment(
            start_ms=4000, end_ms=6000, text="A cold dip restores health and zest."
        ),
    ]
...
def mock_service_response(mock_transcription_result):
    ...
    mock_response.summary = MockSummary(
        total_items=1, processed=1, skipped=0, failed=0, elapsed_time=5.2
    )
...
class TestTranscriptionEndpoint:
    ...
    def test_single_file_json_output(
        self, mock_service_class, client, mock_service_response
    ):
    ...
    @patch("beige_book.api.TranscriptionService")
    def test_single_file_csv_output(
        self, mock_service_class, client, mock_service_response
    ):
...
"""
            ...
            request_data = {
                "input": {"type": "file", "source": "/path/to/audio.mp3"},
                "processing": {"model": "base", "verbose": True},
                "output": {"format": "csv"},
            }
    ...
    @patch("beige_book.api.TranscriptionService")
    def test_single_file_toml_output(
        self, mock_service_class, client, mock_service_response
    ):
...
"""
    ...
    def test_sqlite_output(self, mock_service_class, client, mock_service_response):
        ...
        request_data = {
            ...
            "processing": {"model": "medium", "verbose": True},
            "output": {
                "format": "sqlite",
                "database": {
                    "db_path": "/tmp/test.db",
    ...
    def test_feed_processing(self, mock_service_class, client, mock_service_response):
        ...
        mock_response_multi.summary = Mock(
            total_items=3, processed=3, skipped=0, failed=0, elapsed_time=15.6
        )
...
class TestValidation:
    ...
    def test_missing_required_fields(self, client):
        ...
        # Missing input field
        request_data = {"processing": {"model": "tiny"}, "output": {"format": "json"}}
        response = client.post("/transcribe", json=request_data)
...
class TestEdgeCases:
    ...
    def test_large_model_selection(self, client):
        ...
        with patch("beige_book.api.TranscriptionService") as mock_service_class:
            ...
            request_data = {
                "input": {"type": "file", "source": "/path/to/file"},
                "processing": {"model": "large", "verbose": True},
                "output": {"format": "json"},
            }
    ...
    def test_output_destination_field(self, client):
        ...
        with patch("beige_book.api.TranscriptionService") as mock_service_class:
            ...
            request_data = {
                "input": {"type": "file", "source": "/path/to/file"},
                "processing": {"model": "tiny"},
                "output": {"format": "json", "destination": "/tmp/output.json"},
            }
...
class TestJSONEncoding:
    ...
    def test_json_response_encoding(
        self, mock_service_class, client, mock_transcription_result
    ):
