    ...
    for profile in profiles:
        ...
        if statements:
            ...
            for stmt in statements:
                quote = stmt['text'][:60] + "..." if len(stmt['text']) > 60 else stmt['text']
                print(f"   - \"{quote}\"")
...
def batch_processing_example():
    ...
    for profile in sorted(profiles, key=lambda x: x['total_duration'], reverse=True):
        pct = (profile['total_duration'] / sum(p['total_duration'] for p in profiles)) * 100
        print(f"\n👤 {profile['display_name']}:")
        print(f"   Episodes: {profile['total_appearances']}")
        print(f"   Speaking time: {profile['total_duration'] / 60:.1f} min ({pct:.1f}%)")
