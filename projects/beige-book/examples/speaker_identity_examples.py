...
def example_1_basic_speaker_identification():
    ...
    trans_id = db.save_transcription(
        result, feed_url="https://example.com/podcast/feed.rss"
    )
...
def example_2_preregister_known_speakers():
    ...
    host_id = db.create_speaker_profile(
        display_name="Sarah Johnson", feed_url=feed_url, canonical_label="HOST"
    )
    print(f"Created host profile with ID: {host_id}")

    # Register a regular co-host
    cohost_id = db.create_speaker_profile(
        display_name="Mike Chen", feed_url=feed_url, canonical_label="COHOST"
    )
    ...
    host_embedding, quality = extractor.extract_embedding_from_file(
        "host_intro.wav", start_time=0.0, end_time=10.0
    )
...
def example_3_query_speaker_history():
    ...
    history = db.get_speaker_history(
        profile_id=speaker["id"], start_date=start_date, end_date=end_date
    )
...
def example_6_merge_duplicate_profiles():
    ...
    profile1 = db.create_speaker_profile(
        "John Smith", feed_url="https://example.com/feed"
    )
    profile2 = db.create_speaker_profile("John S.", feed_url="https://example.com/feed")

    print("Created potentially duplicate profiles:")
    ...
    success = matcher.merge_speaker_profiles(
        profile_id_keep=profile1, profile_id_merge=profile2
    )
...
def example_7_cross_episode_tracking():
    ...
    episodes = ["episode_001.mp3", "episode_002.mp3", "episode_003.mp3"]
...
def example_8_speaker_embeddings_analysis():
    ...
    if len(embeddings) >= 2:
        ...
        emb1 = deserialize_embedding(
            emb1_data["embedding"], emb1_data["embedding_dimension"]
        )
        emb2 = deserialize_embedding(
            emb2_data["embedding"], emb2_data["embedding_dimension"]
        )

        similarity = cosine_similarity(emb1, emb2)
        print(f"\nSimilarity between first two embeddings: {similarity:.3f}")
        print(
            f"Quality scores: {emb1_data['quality_score']:.2f}, {emb2_data['quality_score']:.2f}"
        )
        print(
            f"Extraction methods: {emb1_data['extraction_method']}, {emb2_data['extraction_method']}"
        )
...
def main():
    ...
    print(
        "In production, use real audio files and set SPEAKER_EMBEDDING_METHOD='speechbrain'"
    )
