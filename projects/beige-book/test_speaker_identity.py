...
def demonstrate_speaker_identity():
    ...
    with tempfile.NamedTemporaryFile(suffix="_speaker_identity.db", delete=False) as tmp:
        db_path = tmp.name
    ...
    cohost_id = db.create_speaker_profile(
        display_name="Jane Smith (Co-host)",
        feed_url=feed_url,
    ...
    with db._get_connection() as conn:
        ...
        for occ in occurrences:
            print(f"   - {occ['temporary_label']} → {occ['display_name']} "
