...
class SpeakerMatcher:
    ...
    def find_best_match(
        ...
        if feed_url:
            ...
            with self.db._get_connection() as conn:
                cursor = conn.cursor()
                cursor.execute("SELECT * FROM speaker_profiles ORDER BY total_appearances DESC LIMIT 100")
    ...
    def match_speaker(
        ...
        if create_if_not_found:
            # Create new profile
            display_name = speaker_hint or f"Unknown Speaker {datetime.now().strftime('%Y%m%d_%H%M%S')}"
    ...
    def _update_segment_profiles(
        self,
        transcription_id: int,
        speaker_mappings: Dict[str, Tuple[int, float]]
    ):
