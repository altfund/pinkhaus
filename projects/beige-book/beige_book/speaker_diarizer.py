...
class SpeakerDiarizer:
    ...
    def diarize_file(
        ...
        return DiarizationResult(
            segments=segments, num_speakers=len(speakers), audio_duration=duration
        )
    ...
    def _mock_diarization(self, audio_path: str) -> DiarizationResult:
        ...
        while current_time < duration:
            ...
            segment_duration = min(
                5 + torch.rand(1).item() * 10, duration - current_time
            )
        ...
        return DiarizationResult(
            segments=segments, num_speakers=2, audio_duration=duration
        )
        ...
        for trans_seg in transcription_segments:
            ...
            for speaker_seg in diarization.segments:
                ...
                if speaker_seg.start < end and speaker_seg.end > start:
                    overlap_duration = min(speaker_seg.end, end) - max(
                        speaker_seg.start, start
                    )
...
def create_speaker_aware_transcription(
    ...
    enhanced_result = (
        trans_dict.copy()
        if hasattr(transcription_result, "to_dict")
        else transcription_result.copy()
    )
