...
def query_model(client: OllamaClient, model: str, prompt: str, chat: bool = False) -> None:
    try:
        if chat:
...
def ask_command(args):
    ...
    embeddings = EmbeddingService(client, args.embedding_model)
    if not embeddings.ensure_model_available():
        print(f"Error: Embedding model '{args.embedding_model}' not found.", file=sys.stderr)
...
def main():
    ...
    query_parser.add_argument("--chat", action="store_true",
                            help="Use chat mode instead of generate mode")
    
    # Index command
    index_parser = subparsers.add_parser("index", help="Index podcast transcriptions")
    index_parser.add_argument("--db", default="../beige-book/protobuf_transcriptions.db",
                            help="Path to transcriptions database")
    index_parser.add_argument("--vector-store", default="./grant_chroma_db",
                            help="Path to vector store directory")
    index_parser.add_argument("--embedding-model", default="nomic-embed-text",
                            help="Embedding model to use")
    index_parser.add_argument("--chunk-size", type=int, default=512,
                            help="Chunk size in tokens")
    index_parser.add_argument("--chunk-overlap", type=int, default=128,
                            help="Chunk overlap in tokens")
    index_parser.add_argument("--batch-size", type=int, default=10,
                            help="Batch size for indexing")
    index_parser.add_argument("--transcription-id", type=int,
                            help="Index specific transcription ID")
    
    # Ask command
    ask_parser = subparsers.add_parser("ask", help="Ask questions about podcast content")
    ask_parser.add_argument("question", help="Question to ask")
    ask_parser.add_argument("--db", default="../beige-book/protobuf_transcriptions.db",
                          help="Path to transcriptions database")
    ask_parser.add_argument("--vector-store", default="./grant_chroma_db",
                          help="Path to vector store directory")
    ask_parser.add_argument("--model", default="llama3.2",
                          help="LLM model to use for answering")
    ...
    ask_parser.add_argument("--transcription-id", type=int,
                          help="Limit to specific transcription")
    ...
    stats_parser.add_argument("--db", default="../beige-book/protobuf_transcriptions.db",
                            help="Path to transcriptions database")
    stats_parser.add_argument("--vector-store", default="./grant_chroma_db",
                            help="Path to vector store directory")
