...
class SignalServicer(external_pb2_grpc.SignalServiceServicer):
    ...
    def __init__(self, ollama_client: OllamaClient, vector_store_path: str = "../../resources/fc/grant_chroma_db"):
    ...
    def GetProbabilities(self, request, context):
        ...
        for signal_request in request.requests:
            try:
                ...
                while True:
                    if i > 0:
                        print(f"[GRPC] Retry {i} for request {signal_request.source_id}")
                    ...
                    if probability is not None and 0 <= probability <= 1.0:
                        ...
                        logger.error(f"Error processing request grant failed 5 times to produce a probability.")
                    ...
                    logger.error(f"Grant retry {i}, was unable to produce an answer with a probability.")
                    i += 1
...
def serve(port: int = 50051, ollama_base_url: str = "http://localhost:11434"):
    ...
    print(f"[GRPC] Server ready to accept connections...")
