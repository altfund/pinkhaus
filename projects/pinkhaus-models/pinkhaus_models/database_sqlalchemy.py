...
class TranscriptionDatabase:
    ...
    def get_segments_for_transcription(
        ...
        with self.engine.get_session() as session:
            stmt = (
                select(TranscriptionSegmentORM)
                .where(TranscriptionSegmentORM.transcription_id == transcription_id)
    ...
    def get_transcription(
        ...
        with self.engine.get_session() as session:
            ...
            segments = (
                session.query(TranscriptionSegmentORM)
    ...
    def save_transcription(
        ...
        with self.engine.get_session() as session:
            ...
            existing = (
                session.query(TranscriptionMetadataORM)
                .filter_by(file_hash=result.file_hash, model_name=model_name)
                .first()
            )
    ...
    def search_transcriptions(
        ...
        with self.engine.get_session() as session:
            stmt = (
                select(TranscriptionMetadataORM)
                .where(TranscriptionMetadataORM.full_text.like(f"%{query}%"))
                .order_by(
                    desc(TranscriptionMetadataORM.feed_item_published),
                    desc(TranscriptionMetadataORM.created_at),
                )
    ...
    def find_by_hash(
        self, file_hash: str, metadata_table: str = None
    ) -> List[Dict[str, Any]]:
        """Find all transcriptions for a given file hash."""
        with self.engine.get_session() as session:
            stmt = (
                select(TranscriptionMetadataORM)
                .where(TranscriptionMetadataORM.file_hash == file_hash)
    ...
    def check_feed_item_exists(
        ...
        with self.engine.get_session() as session:
            exists = (
                session.query(TranscriptionMetadataORM)
                .filter_by(feed_url=feed_url, feed_item_id=feed_item_id)
                .first()
                is not None
            )
    ...
    def get_transcriptions_by_date_range(
        ...
        with self.engine.get_session() as session:
            ...
            if start_date:
                query = query.filter(
                    TranscriptionMetadataORM.feed_item_published >= start_date
                )

            if end_date:
                query = query.filter(
                    TranscriptionMetadataORM.feed_item_published <= end_date
                )
    ...
    def _orm_to_metadata(
        self, orm_obj: TranscriptionMetadataORM
    ) -> TranscriptionMetadata:
    ...
    def _orm_to_dict(self, orm_obj) -> Dict[str, Any]:
        ...
        if isinstance(orm_obj, TranscriptionMetadataORM):
            return {
                ...
                if orm_obj.feed_item_published
                else None,
                "created_at": orm_obj.created_at.isoformat()
                if orm_obj.created_at
                else None,
            }
        ...
        with self.engine.get_session() as session:
            ...
            for chunk in chunks:
                ...
                if with_embeddings and chunk.embedding is not None:
                    chunk_dict["embedding"] = np.frombuffer(
