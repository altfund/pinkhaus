...
class TranscriptionMetadataORM(Base):
    ...
    __table_args__ = (
        ...
        Index(
            "idx_transcription_metadata_feed_item",
            "feed_url",
            "feed_item_id",
            unique=True,
            postgresql_where=(feed_url.isnot(None) & feed_item_id.isnot(None)),
        ),
    )
...
class TranscriptionSegmentORM(Base):
    ...
    transcription_id = Column(
        Integer, ForeignKey("transcription_metadata.id"), nullable=False
    )
...
class TextChunkORM(Base):
    ...
    # Indexes
    __table_args__ = (Index("idx_chunks_created", "created_at"),)
