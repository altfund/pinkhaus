#!/usr/bin/env python3
"""
Enhanced Web Monitor with Authentication and Rate Limiting
...
def before_request():
    ...
    if request.path in ['/health', '/api/health', '/api/docs']:
        return
...
def get_portfolio():
    ...
    try:
        ...
        with get_paper_session() as session:
            ...
            open_value = sum(p.stake for p in open_positions)
...
def main():
    ...
    logger.info(f"Starting authenticated web monitor on {host}:{port}")
