...
def normalize_outcome(bookmaker: str, market_type: str, outcome: str) -> str:
    ...
    OUTCOME_MAPPING = {
        "overtime_markets": {
            "total": {"option_1": "Over", "option_2": "Under"},
            "winner": {"option_1": "Home", "option_2": "Away", "option_3": "Draw"},
            "spread": {"option_1": "Home", "option_2": "Away"},
        },
        "pinnacle": {
    ...
    return OUTCOME_MAPPING.get(bookmaker, {}).get(market_type, {}).get(outcome, outcome)
...
def add_implied_probabilities(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df["implied_normalized"] = df.groupby(group_cols)["implied_raw"].transform(
...
def evaluate_odds_edges(
    latest_odds: pd.DataFrame, edge_threshold: float = 1.5
) -> pd.DataFrame:
    """
    ...
    pinnacle_subset = pinnacle[
        merge_keys + ["implied_normalized", "implied_raw"]
    ].rename(
        columns={
            "implied_normalized": "pinnacle_implied_normalized",
            "implied_raw": "pinnacle_implied_raw",
        }
    )

    # Step 3: Merge Pinnacle normalized probabilities into Overtime rows
    overtime = pd.merge(overtime, pinnacle_subset, on=merge_keys, how="left")

    # Step 4: Calculate edge metrics
    overtime["edge_vs_pinnacle"] = (
        overtime["pinnacle_implied_normalized"] - overtime["implied_raw"]
    )
    overtime["internal_edge"] = overtime["implied_normalized"] - overtime["implied_raw"]
    overtime["vig_diff"] = (
        overtime["implied_normalized"] - overtime["pinnacle_implied_normalized"]
    )
    overtime["raw_implied_diff"] = (
        overtime["implied_raw"] - overtime["pinnacle_implied_raw"]
    )

    # Step 5: Flag meaningful edges
    overtime["flag_edge_vs_pinnacle"] = (
        overtime["edge_vs_pinnacle"].abs() >= edge_threshold
    )
    ...
    overtime["flag_raw_implied_diff"] = (
        overtime["raw_implied_diff"].abs() >= edge_threshold
    )
    ...
    pivot_cols = [
        ...
        "implied_raw",
        "implied_normalized",
        "pinnacle_implied_normalized",
        "pinnacle_implied_raw",
        "edge_vs_pinnacle",
        "internal_edge",
        "vig_diff",
        "raw_implied_diff",
        "flag_edge_vs_pinnacle",
        "flag_internal_edge",
        "flag_vig_diff",
        "flag_raw_implied_diff",
    ]
...
def generate_signals(
    edge_summary: pd.DataFrame, edge_threshold: float = 1.5, vig_tolerance: float = 0.25
) -> pd.DataFrame:
    """
    ...
    score_edge_vs_pinnacle = (df["edge_vs_pinnacle"] > edge_threshold).astype(
        int
    )  # True price gap
    score_vig_alignment = (df["vig_diff"].abs() < vig_tolerance).astype(
        int
    )  # Signal clarity (vig not distorting)
    score_price_advantage = (df["edge_vs_pinnacle"] > 0).astype(
        int
    )  # Overtime paying better than fair
    # score_internal_ignore = 0  # We no longer trust overtime's own normalized probs for signal

    # Total score
    df["pinnacle_bet_signal_score"] = (
        score_edge_vs_pinnacle + score_price_advantage + score_vig_alignment
    )
...
def generate_internal_mispricing_signals(
    edge_summary: pd.DataFrame,
    internal_edge_threshold: float = 1.5,
    max_implied_raw: float = 60.0,
    max_internal_vig: float = 5.0,
) -> pd.DataFrame:
    """
    ...
    overrounds = (
        df.groupby(group_keys)["implied_raw"].sum().reset_index(name="overround")
    )
    overrounds["internal_vig"] = overrounds["overround"] - 100

    # Merge internal vig back to the odds
    df = pd.merge(
        df, overrounds[group_keys + ["internal_vig"]], on=group_keys, how="left"
    )
    ...
    df["internal_signal_score"] = (
        score_internal_edge + score_reasonable_odds + score_vig_clarity
    )
