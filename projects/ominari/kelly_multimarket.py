...
def calculate_kelly_utility(stakes, odds, probs, bankroll, correlation_matrix=None, exclusivity_groups=None):
    """
    ...
    if correlation_matrix is not None:
        for i, j in itertools.combinations(range(len(stakes)), 2):
            ...
            total_utility -= 0.01 * corr * stake_prod  # penalize correlation overlap slightly

    return -total_utility  # because we're minimizing


def optimize_kelly_multimarket(bets, bankroll, exclusivity_groups=None, correlation_matrix=None):
    """
    ...
    if exclusivity_groups:
        ...
        for group, indices in exclusivity_groups.items():
            ...
            best_index = None
            for i in indices:
                ev = bets[i]['prob'] * (bets[i]['odds'] - 1) - (1 - bets[i]['prob'])  # basic EV
                if ev > best_ev:
                    best_ev = ev
                    best_index = i
...
def calculate_kelly_stakes_with_exclusivity(
    ...
    def kelly_objective(fractions):
        ...
        """
        expected_returns = bets_df['probability'].values * np.log1p(fractions * (bets_df['odds'].values - 1))
    ...
    exclusivity_groups = {}
    group_ids = bets_df.groupby(['source_id', 'unified_market_type', 'normalized_line']).indices
    ...
    for indices in exclusivity_groups.values():
        constraints.append({
            'type': 'ineq',
            'fun': lambda f, idx=indices: 1 - np.sum(f[idx])
        })
    ...
    return bets_df[['source_id', 'unified_market_type', 'normalized_outcome', 
                    'normalized_line', 'market_name', 'league_name', 'bookmaker',
                    'odds', 'probability', 'stake', 'stake_fraction']]
...
if __name__ == "__main__":
    ...
    bets = [
        {'name': 'Over 2.5 goals', 'odds': 2.0, 'prob': 0.5, 'match_id': 'match123', 'unified_market_type': 'total_goals', 'line':2.5},
        {'name': 'Under 2.5 goals', 'odds': 2.0, 'prob': 0.5, 'match_id': 'match123', 'unified_market_type': 'total_goals', 'line':2.5},
        {'name': 'Team A to win', 'odds': 3.0, 'prob': 0.3, 'match_id': 'match123', 'unified_market_type': 'winner', 'line':0.0},
        {'name': 'Team B to win', 'odds': 2.5, 'prob': 0.4, 'match_id': 'match123', 'unified_market_type': 'winner', 'line':0.0},
        {'name': 'Draw', 'odds': 3.5, 'prob': 0.3, 'match_id': 'match123', 'unified_market_type': 'winner', 'line':0.0},
    ]
    ...
    result = optimize_kelly_multimarket(bets, bankroll, exclusivity_groups, correlation_matrix)
    print("Recommended Bets:")
    for r in result:
        print(f" - {r['name']} @ {r['odds']} | Prob: {r['prob']} | Stake: ${r['stake']}")
