...
# Use SQLite directly since PostgreSQL has issues
import sqlite3
...
# Import the original for compatibility but we'll use SQLite
try:
...
def get_market_data():
    ...
    try:
        with db_manager.get_db_session() as db:
            # Get active Soccer markets first, then others
            ...
            active_markets = soccer_markets + other_markets
            ...
            for market in active_markets:
                ...
                signal_text = None
                is_in_play = False  # Initialize is_in_play variable
                ...
                # Only include markets with at least home and away odds for proper display
...
@app.route('/api/test-markets')
def test_markets():
    """Test endpoint to verify market data"""
    try:
        markets, signals, stats, chunks = get_market_data()
        return jsonify({
            'success': True,
...
def api_unified_dashboard():
    ...
    try:
        ...
        # Markets with signals - use get_market_data() for consistency
        markets_list = []
        try:
            # Get sport filter from query params
            ...
            if sport_filter and sport_filter != 'all':
                markets = [m for m in markets if m.get('sport', '').lower() == sport_filter.lower()]
            
            # Use the markets from get_market_data which already have proper odds
        ...
        return jsonify({
            ...
            'system': system_stats,
            'markets': market_data.get('markets', []),  # No limit - let frontend handle pagination
