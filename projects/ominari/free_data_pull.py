...
def create_or_replace_table(database, df, table_name):
    if df.empty:
        logging.warning(
            f"No data available to write to {table_name}. Skipping table update."
        )
        return
                    ...
                    results.append(
                        {
                            ...
                            "position_names": json.dumps(
                                market.get("positionNames") or []
                            ),
                        }
                    )
...
def fetch_events(sport_key, api_key=ODDS_API_KEY):
    try:
        ...
        logging.info(
            f"Fetching events using the Odds API /events endpoint for sport_key={sport_key}..."
        )
        response = requests.get(url)
        response.raise_for_status()
        return response.json()
        ...
        if response.status_code == 422:
            logging.warning(
                f"Skipping unsupported sport_key: {sport_key}. Full URL: {url}"
            )
        else:
            logging.error(
...
def upsert_teams_from_markets(conn, markets_df):
    ...
    for _, m in markets_df.iterrows():
        ...
        teams.append({"team_name": home, "league": league, "updated_at": now})
        teams.append({"team_name": away, "league": league, "updated_at": now})
...
def upsert_teams_from_games(conn, games_df):
    ...
    for _, g in games_df.iterrows():
        teams.append(
            {"team_name": g["home_team"], "league": g["league"], "updated_at": now}
        )
        teams.append(
            {"team_name": g["away_team"], "league": g["league"], "updated_at": now}
        )
    teams_df = pd.DataFrame(teams).drop_duplicates(subset=["team_name"])
...
if __name__ == "__main__":
    ...
    if needs_update(last_updated):
        ...
        if overtime_market_all_json:
            ...
            overtime_markets_odds_df = get_overtime_markets_odds(
                overtime_all_json=overtime_market_all_json
            )  # overtime_market_all_df
            ...
            with sqlite3.connect(DB_NAME) as conn:
                ...
                enriched = trimmed.merge(
                    games_df[game_fields + ["source_id"]], on="source_id", how="left"
                )
