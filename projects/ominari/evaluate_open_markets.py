...
def get_upcoming_overtime_markets_with_signals(
    ...
    df["normalized_outcome"] = df.apply(
        lambda r: normalize_outcome(r.bookmaker, r.market_type, r.outcome), axis=1
    )
    ...
    mask = (df["maturity_date"] >= chunk_start) & (df["maturity_date"] <= chunk_end)
    df = df.loc[mask]
    ...
    t7 = time.time()
    df = df.sort_values("time", ascending=False).drop_duplicates(
        subset=[
            "unified_market_type",
            "normalized_outcome",
            "normalized_line",
...
def prepare_kelly_input(
    open_markets: pd.DataFrame, signal_providers: None, signal_weights: None
) -> pd.DataFrame:
    """
    ...
    print(
        f"[DEBUG] prepare_kelly_input called with {len(open_markets)} rows; "
...
def summarize_match_schedule_from_open_markets(
    ...
    df["maturity_date"] = pd.to_datetime(df["maturity_date"], errors="coerce", utc=True)
...
def extract_active_game_periods_from_breaks(
    ...
    if match_df.empty:
        ...
        return pd.DataFrame(
            columns=["chunk_start", "chunk_end", "duration_minutes", "num_games"]
        )
            ...
            [
                {
                    ...
                    "duration_minutes": (
                        df["end_time"].iloc[-1] - df["maturity_date"].iloc[0]
                    ).total_seconds()
                    / 60,
                    "num_games": len(df),
                }
        ...
        for j in range(i + 1, n):
            ...
            if type_i == type_j == "total":
                if outcome_i == outcome_j:
                    corr[i, j] = corr[j, i] = (
                        nested_corr_base - nested_corr_decay * abs(line_i - line_j)
                    )
...
def apply_overtime_fees(df: pd.DataFrame, default_skew: float = 0.01) -> pd.DataFrame:
    ...
    df.loc[is_overtime, "total_fee_pct"] = (
        df.loc[is_overtime, "safebox_fee"] + df.loc[is_overtime, "skew_fee"]
    )
...
def calculate_expected_kelly_return(
    kelly_results: pd.DataFrame, correlation_matrix: np.ndarray
) -> float:
    """
...
def trim_kelly_results(
    ...
    # Cap per game + market
    per_game_market = df.groupby(["source_id", "unified_market_type"])[
        "stake_fraction"
    ].transform("sum")
    scale_gm = (
        (cap_per_game_market / per_game_market).clip(upper=1.0, lower=0.0).fillna(1.0)
    )
    df["stake_fraction"] *= scale_gm
    df.loc[per_game_market > cap_per_game_market, "trim_reason"] += (
        "|cap_per_game_market"
    )
    ...
    display(
        df[["market_name", "odds", "probability", "original_stake"]].sort_values(
            "original_stake", ascending=False
        )
    )
    display(
        df[["market_name", "unified_market_type", "normalized_outcome"]].sort_values(
            "market_name", ascending=False
        )
    )
...
def generate_betting_session_report_and_save(
    ...
    breaks_df = find_upcoming_game_breaks(
        match_df, min_break_minutes, avg_game_duration_minutes, now=as_of
    )
    ...
    if not game_times.empty:
        ...
        if len(game_times) > 1:
            ...
            end = first_chunk["chunk_start"] + timedelta(
                minutes=avg_game_duration_minutes
            )

    markets_df = open_markets[
        (open_markets["maturity_date"] >= start) & (open_markets["maturity_date"] < end)
    ]
...
def save_and_display_betting_report(
    report_data: dict, base_dir: str = "betting_reports", display_md: bool = True
) -> dict:
    """
