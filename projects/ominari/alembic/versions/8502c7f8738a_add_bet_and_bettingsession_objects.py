...
def upgrade():
    ...
    session_enum = sa.Enum(
        'backtest', 'paper', 'live', 'simulation',
    ...
    if 'bet' not in existing:
        op.create_table(
            'bet',
            sa.Column('id',                  sa.Integer(), primary_key=True, autoincrement=True),
            sa.Column('session_id',          sa.Integer(), sa.ForeignKey('betting_session.id'), nullable=False, index=True),
            sa.Column('source_id',           sa.String(),  sa.ForeignKey('market.source_id'),    nullable=False),
            ...
            sa.Column('created_at',          sa.DateTime(), server_default=sa.func.current_timestamp()),
            sa.UniqueConstraint(
                'session_id', 'source_id', 'unified_market_type',
