#!/usr/bin/env python3
"""
Migrate blockchain data from SQLite to PostgreSQL (public schema)
"""
        ...
        with postgres_engine.connect() as pg_conn:
            for market in markets:
                try:
                    ...
                    logger.debug(f"Error inserting market: {e}")
            
            pg_conn.commit()
            logger.info("✅ Markets migrated")
            ...
            for odd in odds:
                try:
                    ...
                    """), {
                        ...
                        'bookmaker': odd[5],
                        'updated_at': odd[6],
                        'position': odd[7] if odd[7] is not None else 0
                    })
                except Exception as e:
                    logger.debug(f"Error inserting odd: {e}")
            
            pg_conn.commit()
            logger.info("✅ Odds migrated")
