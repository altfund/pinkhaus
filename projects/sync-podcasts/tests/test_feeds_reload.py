...
""")
    ...
    def test_feeds_reload_between_cycles(self, temp_feeds_file, temp_db):
        ...
        with patch(
            "beige_book.feed_parser.FeedParser.parse_all_feeds",
            side_effect=mock_parse_all_feeds,
        ):
...
""")
    ...
    def test_concurrent_instances_different_feeds(self, temp_db):
        ...
        try:
            config1 = SyncConfig(
                feeds_path=feeds1, db_path=temp_db, skip_validation=True
            )
            config2 = SyncConfig(
                feeds_path=feeds2, db_path=temp_db, skip_validation=True
            )
            ...
            with patch("sync_podcasts.sync.TranscriptionService"):
                with patch("sync_podcasts.sync.OllamaClient"):
                    ...
                    assert (
                        syncer1.process_lock.lock_file != syncer2.process_lock.lock_file
                    )
