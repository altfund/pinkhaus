...
""")
    ...
    def test_startup_report_with_unprocessed_items(
        self, temp_db, temp_feeds_file, caplog
    ):
        ...
        config = SyncConfig(
            feeds_path=temp_feeds_file, db_path=temp_db, skip_validation=True
        )
        ...
        with patch("sync_podcasts.sync.TranscriptionService"):
            with patch("sync_podcasts.sync.OllamaClient"):
                with patch(
                    "beige_book.feed_parser.FeedParser.parse_feed",
                    side_effect=mock_parse_feed,
                ):
    ...
    def test_startup_report_with_failed_items(self, temp_db, temp_feeds_file, caplog):
        ...
        with patch("sync_podcasts.sync.TranscriptionService"):
            with patch("sync_podcasts.sync.OllamaClient"):
                with patch(
                    "beige_book.feed_parser.FeedParser.parse_feed", return_value=[]
                ):
                    with caplog.at_level(logging.INFO):
                        _ = PodcastSyncer(config)
