    ...
    def validate_feed(self, feed_url: str) -> FeedValidationResult:
        ...
        try:
            ...
            if feed.bozo:
                if hasattr(feed, "bozo_exception"):
                    ...
                    if any(err in error_msg.lower() for err in [
                        "urlopen error", "connection", "timeout", 
                    ...
                    if "not well-formed" in error_msg.lower() or "syntax error" in error_msg.lower():
                        return FeedValidationResult(
                            is_valid=False,
            ...
            if not hasattr(feed, 'feed') and not hasattr(feed, 'entries'):
                return FeedValidationResult(
                    is_valid=False,
                    error_message="No feed data found at URL"
                )
...
def validate_feeds_toml(toml_path: str, move_invalid: bool = False) -> Dict[str, FeedValidationResult]:
    """
    ...
    try:
        ...
        return {"_error": FeedValidationResult(
            is_valid=False,
            error_message=f"Error parsing TOML file: {str(e)}"
        )}
...
def create_invalid_feeds_toml(
    ...
    """
    # Check if invalid-feeds.toml already exists
    existing_data = {}
    ...
    if "feeds" not in existing_data:
        existing_data["feeds"] = {}
    
    # Add invalid feeds with their error messages as comments
    invalid_feeds_with_errors = []
    for feed_url in invalid_feeds:
        result = results.get(feed_url)
        error_msg = result.error_message if result else "Unknown error"
        # Store as a dict to preserve the error information
        invalid_feeds_with_errors.append({
            "url": feed_url,
            "error": error_msg,
            "title": result.feed_title if result and result.feed_title else None
        })
    
    # Store the invalid feeds
    existing_data["feeds"]["invalid_rss"] = invalid_feeds_with_errors
    ...
    with open(invalid_toml_path, 'w') as f:
        ...
        f.write("[feeds]\n")
        f.write("invalid_rss = [\n")
        for item in invalid_feeds_with_errors:
