...
class PodcastSyncer:
    ...
    def _show_startup_report(self):
        ...
        if failed_summary:
            logger.warning(f"Failed items found in 'failed_items' table:")
        ...
        try:
                ...
                try:
                    ...
                    for item in items:
                        if not self.db.check_feed_item_exists(
                            ...
                            failed_info = self.db.get_failed_item(feed_url, item.item_id)
                            if not failed_info or failed_info['failure_count'] < self.config.max_failures:
                                unprocessed_count += 1
            ...
            if feed_stats:
                logger.warning(
                    f"Unprocessed items found (will be stored in 'transcription_metadata' table):"
                )
    ...
    def _should_skip_item(self, feed_url: str, feed_item_id: str) -> tuple[bool, str]:
        ...
        if failed_info and failed_info['failure_count'] >= self.config.max_failures:
            return True, f"permanently failed after {failed_info['failure_count']} attempts"
