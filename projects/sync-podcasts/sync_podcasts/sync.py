...
from beige_book.models import TranscriptionRequest, InputConfig, ProcessingConfig, OutputConfig, FeedOptions, DatabaseConfig
...
class PodcastSyncer:
    ...
    def __init__(self, config: SyncConfig = SyncConfig()):
        ...
        if config.days_back:
            self.date_threshold = (datetime.now() - timedelta(days=config.days_back)).isoformat()
    ...
    def process_one_podcast(self) -> bool:
        ...
        if not response.success or not response.summary or response.summary.processed == 0:
    ...
    def _run_once(self):
        ...
        if processed_count > 0:
            logger.info(f"Sync completed successfully! Processed {processed_count} podcast(s).")
            print(f"\nYou can now query the podcasts using:")
            print(f"  flox activate -- uv run python -m grant ask \"your question here\" --vector-store {self.config.vector_store_path}")
        else:
            logger.info("No new podcasts to process.")
