...
updatepath<-paste0(basepath,"/data/combo/to_process/")
file<-list.files(updatepath) #should be just 1 but just in case
data<-read.csv(paste0(updatepath,file[1]))
...
#laliga$Div<-as.character(laliga$Div)
...
Season = as.numeric(factor(d$Season, levels = seasons)), #Div = as.numeric(factor(d$Div, levels = divs)),
    n_teams = length(teams),
n_games = nrow(d), n_seasons = length(seasons))#, n_divs = length(divs))
...
#m3 <- jags.model(textConnection(m3_string), data = data_list, n.chains = 3,n.adapt = 1000)
#update(m3, 1000)
#s3 <- coda.samples(m3, variable.names = c("home_baseline", "away_baseline","skill", "season_sigma", "group_sigma","group_skill"), n.iter = 400, thin = 4)
#ms3 <- as.matrix(s3)
...
folder<-format(Sys.Date(),format="%Y%m%d")

recentname<-paste0(format(Sys.Date(),format="%Y%m%d"),"_modelrun")
updatepath<-paste0(basepath,"/model/results/",folder,"/",recentname,".csv")
write.table(ms3,updatepath,sep=",",row.names=FALSE)

recentname<-paste0(format(Sys.Date(),format="%Y%m%d"),"_teams")
updatepath<-paste0(basepath,"/model/results/",folder,"/",recentname,".csv")
write.table(teams,updatepath,sep=",",row.names=FALSE)

recentname<-paste0(format(Sys.Date(),format="%Y%m%d"),"_seasons")
updatepath<-paste0(basepath,"/model/results/",folder,"/",recentname,".csv")
write.table(seasons,updatepath,sep=",",row.names=FALSE)
